...
- `runtime/migrations/` — 35 tables on Postgres 16, row-level security forced on all 31 tenant-scoped ones
...
- `tests/` — 1116 tests, each backing a specific claim made in `docs/`; 364 of them run against a real Postgres (`pytest -m db`) and CI fails a run that skipped them
...
PYTHONPATH=. python3 -m pytest tests/ -q               # 1116 tests
