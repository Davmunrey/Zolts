...
- `runtime/migrations/` — 30 tables on Postgres 16, row-level security forced on all 27 tenant-scoped ones
...
- `tests/` — 875 tests, each backing a specific claim made in `docs/`; 300 of them run against a real Postgres (`pytest -m db`) and CI fails a run that skipped them
...
PYTHONPATH=. python3 -m pytest tests/ -q               # 875 tests
