...
- `tests/` — 709 tests, each backing a specific claim made in `docs/`; 205 of them run against a real Postgres (`pytest -m db`) and CI fails a run that skipped them
...
PYTHONPATH=. python3 -m pytest tests/ -q               # 709 tests
