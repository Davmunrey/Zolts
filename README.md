...
- `runtime/connectors/` — the CRM contract and its suite, HubSpot and Pipedrive as sources, Smartlead for sending
...
- `tests/` — 448 tests, each backing a specific claim made in `docs/`; the 142 runtime tests run against a real Postgres and CI fails a run that skipped them
...
PYTHONPATH=. python3 -m pytest tests/ -q               # 448 tests
