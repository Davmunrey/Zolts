...
- **A test may not depend on ambient time, ambient configuration or ambient data.** Where it must touch one, it establishes the premise itself and asserts that premise before the verdict — open a connection with `sslmode=disable` and check it really is plaintext; remove the send window and check the shipped programme still declares one. Eight defects have been this shape (D-35, D-59, D-68, D-71, D-73, D-74, D-86, D-105); one left the suite red about seventy per cent of the week on code that was never wrong, one had the verification gate itself report eight broken guards on a machine with no database, and one had twenty-four tests — eighteen of them tenant scoping — fail on a clean checkout because the fixtures fell back to a role that bypasses the grants they assert. `scripts/ambient_check.py` runs the suite under named ambient conditions and reports what disagrees, so the probe that found four of the five is no longer something somebody has to remember. **The rule binds the checks as much as the tests:** a script that reports a verdict must establish that it could run, and refuse rather than report a partial one — a file where half the tests skip and half pass looks exactly like a guard surviving. A suite that fails on correct code teaches the team to press re-run, and a team that presses re-run has no suite.
