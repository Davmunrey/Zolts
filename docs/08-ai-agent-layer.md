...
- **Cost target under €0.02 in tokens per contact touched** — *not measured*: every model call is priced before it is made and recorded after it, so the number is computable per programme and per period. Nothing computes it, and nothing compares it to this target. AGENT-4.
