...
| **Read** | 9 | Compared a document against the code, looking for the caller that does not exist |
...
Twenty-five of the thirty-seven are one defect wearing different clothes: **a complete specification with no caller.** A price in `docs/12` nothing charges. A column no code sets. A button labelled for a feature that does not exist. A process table CI never runs. A restore script that ends by telling you to test it.

The second shape, found only once the first was exhausted: **a guard that passes when you break the thing it guards.** Six of those, and two of them were written in the same session that found them.
