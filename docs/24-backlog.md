...
| Tests | 1286 | 403 against a real Postgres; CI fails a run that skipped them |
...
The register in `docs/22` is the evidence for this ordering. Forty-two of seventy defects were a specification with no caller. Reading found more of them than any other method — thirty of the seventy — and every one of those was a *document* read against the code — a price nothing charges, an ADR about a connector that does not exist. Not one defect in the register was found by reviewing code for a wrong line.
