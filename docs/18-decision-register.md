...
| 32 | **A5** how a program excludes accounts with an open deal | Default: it cannot, and the clause is absent from the shipped program rather than written as one that always passes. There is no `opportunity` table and the CRM contract reads accounts and contacts only — adding deals means a protocol method, three connector implementations, a table and a sync. It is the highest-value missing exclusion for a B2B GTM product: outbound into a live deal is the expensive kind of wrong, because it reaches somebody the sales team is already talking to. Until it exists, say so rather than imply otherwise. See ADR-034 |
