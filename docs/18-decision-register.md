...
| 27 | **P11** what a second sending channel costs | Default: it does not exist. `docs/12` prices one sending action, LinkedIn has no public outbound API, and every route to one is a third-party vendor or an automation that breaks a platform's terms — so the price and the vendor are one decision, and it is the founder's. Until it is made, a LinkedIn step is a task for a person rather than a send nobody priced. The seam is built so that making it is a definition and a connector, not a rewrite. See ADR-027 |
