...
| 33 | **A6** what a program's `enrich` block should buy, and under whose vocabulary | Default: nothing yet, and the block stays unread until this is settled. The shipped programs ask for `work_email`, `linkedin_urn`, `title`, `tech_stack`, `headcount_by_dept` and `funding_history`; `zolts/billing.py` prices `enrich.email`, `enrich.phone` and `enrich.firmographics`. There is no overlap, so executing the block today would raise "not a priced field" for every field of every program. Two decisions are tangled here and only one is engineering's: the names can be reconciled by editing the programs, but `tech_stack`, `headcount_by_dept` and `funding_history` have no price at all, and pricing them is the founder's. Until then an operator buys fields with the CLI, which works and does not scale. See D-33 |
