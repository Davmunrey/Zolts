...
| 45 | **C6** what a `max_cost_per_meeting` breach does mid-period | Default: **report it, never act on it automatically.** Unlike a per-account cap, this one divides spend by an outcome that arrives late and in lumps: a programme is over its cost-per-meeting ceiling every day until the first meeting lands, and pausing on that would kill working programmes in their first fortnight. The honest form is a figure on the measurement panel beside lift, with the ceiling drawn, and a signal when it has been breached for a whole reporting period rather than a day. Cost of the default being wrong: a programme burns a month of budget at three times the declared acquisition cost and only the period report says so. Revisit when a partner asks for it to be enforced, which is the conversation where they tell us what they want it to do — the question this default exists to avoid guessing at |
