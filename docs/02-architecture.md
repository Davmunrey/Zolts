...
| `worker`, a loop | `/api/tick`, invoked by Vercel Cron once a minute, drains the outbox in passes until it is empty or a fifty-second budget runs out, and says which | `Worker.tick()` was already invocation-shaped. A pass the platform cuts off leaves actions leased; the lease expires and the idempotency key deduplicates the retry (ADR-005, ADR-007) |
