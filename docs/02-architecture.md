...
| Decision | Why |
|---|---|
| **Every catalogue signal has a row, the silent ones included** | A signal that fired zero times is the one the operator is paying for and getting nothing from. A funnel that showed only the active signals would hide exactly the rows the question is about. A type that fired and is not in the catalogue has a row too, marked as such: money spent on an undefined signal is still money |
| **A conversion counts inside the programme's declared window and nowhere else** | `opportunity_created_90d` means within ninety days of enrolment (`zolts.metrics`). The window is half-open — enrolment is inside, the declared day is not — which is the edge the frozen report draws. An outcome before enrolment is not the signal's doing. Each enrolment is judged by the metric and window of its own programme, resolved exactly as `runtime.reporting` resolves them, so a signal feeding a thirty-day programme and a ninety-day one is judged twice, correctly, rather than once at ninety |
| **The predicate lives in `zolts.signalfunnel` and the reader applies it** | One `in_window`, tested at its edges without a database, and counted from in Python rather than restated in SQL. A window written twice is two windows |
| **Descriptive, not attributed** | The holdout converts too, and it is counted and shown as held out. Whether the programme caused a conversion is the incrementality report's question, asked per programme (ADR-043). `docs/06` SIG-2 and SIG-3 stay *not built* on the instrument they need — lift per signal — and this screen does not pretend to be it |
| **Reached is the treatment arm, by construction** | A control enrolment is never scheduled. The query counts touches on treatment enrolments only, and the rule refuses a row where reached exceeds the arm that can be reached: a reader would not know which column lies |
| **The stages ship with their definitions** | Each stage carries the sentence that says what its number counts, and the screen renders the stages it is sent rather than a second list of names. A number without a definition is a number two people read differently in the same meeting (`docs/28`, OX-10) |
| **A section carries its own column head** | The Signals list holds three sections with three column sets. The view's head belongs to the funnel, which leads; the detected and watched sections render their own, through the same function that strips the renderer's prefixes, so a phone row is labelled by the columns it actually has |
