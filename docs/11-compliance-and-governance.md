...
- **AI content disclosure** — *built, from the pack*: `evals.compliance_checks` refuses a draft carrying no disclosure marker, and until pack v2 nothing ever asked it to — `requires_ai_disclosure` was a keyword defaulting to `False` with no caller that set it (D-90). A jurisdiction rule now carries `ai_disclosure`, the gate resolves it from the **published** pack and hands it to the generator, and the shipped pack requires the marker in the EU jurisdictions it covers and in any jurisdiction it does not (decision 55, ADR-055). Counsel changes that list by republishing a document rather than by waiting for a release. **Default templates are still absent:** the marker is a constraint the copywriter must satisfy, and a template is copy somebody has to write — COMP-4.
