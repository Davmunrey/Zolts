...
| Piece | Where | State |
|---|---|---|
| Schema, 37 tables, RLS forced on 32 | `runtime/migrations/` | Running on Postgres 16 |
...
| The metric registry: what a programme's `primary_metric` counts and for how long | `zolts/metrics.py`, admission, the measurement endpoint, the console, the report | Running; a metric nothing can count is refused where the programme is stored (D-51) |
...
| The report on screen, beside the live figure it was frozen from | `runtime/api/console.py`, the programme detail's *Frozen reports* panel | Running; derived from what the operator already produced, with no input of its own (`docs/17`) |
...
1838 tests. 554 of them run against a real Postgres (`pytest -m db`) and are skipped, never faked, when one is absent — an isolation property verified against a stub is not verified. CI fails a run that skipped them.
