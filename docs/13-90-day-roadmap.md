...
| Runtime | Durable runtime (shipped as a Postgres outbox with leases, ADR-007), DSL v0.1 (`trigger/audience/enrich/score/route/plays/exit`), idempotency | Replay of 10k enrollments with zero duplicate sends |
...
| Commercial | Three design partners signed, paid pilots at €5k for three months | €15k collected; commitment letter with success criteria (the letter and the path are `docs/26`) |
