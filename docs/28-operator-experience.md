...
|---|---|---|
| **An operator does their day inside it** | Not alongside it with a CRM tab, a spreadsheet and a Slack thread open | No: every action reloads the page, nothing updates live, nothing is bulk |
...
| **OX-8** | **A palette that reaches everything, and no dead commands.** Every screen, every contact, every programme, every action from ⌘K. *New program from blueprint* is wired or removed: today its handler is `function(){}` | A command that does nothing is D-38 inside the palette. Keyboard-first is what an operator who lives in a tool expects | `#open-pal`, the blueprints list | A test fails on any palette command whose handler is empty; the browser check reaches the Outbox from the palette | S |
