...
| `plays` | Channel step sequence with waits and branching | Every step emits a `proposed_action`; it never executes directly. A step on `task` or `voice` is work for a person: it carries the deadline `sla_hours` declares, stamped when the task is created so republishing the programme cannot clear a breach, and a person closes it through `zolts task-done` or `POST /v1/tasks/{id}/complete`. Nothing could close one at all until D-83, and the SLA timed nothing until D-84. A human step does **not** gate the sequence — the following steps proceed while the task waits (decision 51) |
